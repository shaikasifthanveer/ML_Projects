...
def predict():
    ...
    try:
        ...
        # 1. Download Data (Using 'max' for better context, similar to your snippet)
        ...
        # 2. OPTIMIZATION: Limit to last 2000 points (Matches your snippet)
        # This keeps training fast while using the most relevant recent trends
        ...
        for i in range(seq_length, len(scaled_features)):
            ...
            y.append(scaled_features[i, 3]) # Target is Close price
        ...
        # 4. IMPROVED MODEL ARCHITECTURE (Matches your snippet)
        model = Sequential([
            # Layer 1: 80 Units + Return Sequences (passes data to next LSTM layer)
            LSTM(80, return_sequences=True, input_shape=(seq_length, 4)),
            Dropout(0.2),
            # Layer 2: 40 Units (Standard LSTM)
            LSTM(40),
            Dropout(0.2),
            # Output Layer
            Dense(1)
        ...
        # 5. EARLY STOPPING
        # Stops training if accuracy doesn't improve for 7 epochs
        ...
        model.fit(X_train, y_train, 
                  ...
                  callbacks=[early_stop], # Adding the callback here
                  verbose=0)
